  ...
  const getJSXExpressionValue = expression => {
    switch (expression.type) {
      ...
      case ast.ARRAY_EXPRESSION:
        return expression.elements.toString();
...
const generateIdValue = (jsxNode, prefix, divider) => {
  ...
  return `${prefix}${divider}${md5(`${jsxNodeName}_${attributes}_${selfClosing}`)}`;
};
...
const checkContext = context => {
  ...
  const reportError = (rootNode, attributeNode) => {
    context.report({
      ...
      fix: fixer => {
        const idValue = generateIdValue(rootNode, prefix, divider);
